...
def list_appointments():
    rows = db.execute(
        """
        SELECT id, title, description, category, duration_mins, is_published,
               payment_requirement, payment_amount, manual_confirmation,
               allow_rescheduling, allow_cancellation, cancellation_cutoff_hours,
               refund_percent_before_cutoff, refund_percent_after_cutoff,
...
def create_appointment():
    ...
    allow_res   = bool(data.get("allow_rescheduling", True))
    allow_can   = bool(data.get("allow_cancellation", True))
    cut_hours   = int(data.get("cancellation_cutoff_hours", 24))
    ref_before  = int(data.get("refund_percent_before_cutoff", 100))
    ref_after   = int(data.get("refund_percent_after_cutoff", 0))
    ...
    appt_id = db.execute(
        ...
        INSERT INTO appointment_types
            (organiser_id, title, description, category, duration_mins, max_capacity,
             payment_requirement, payment_amount, allow_rescheduling, 
             allow_cancellation, cancellation_cutoff_hours, 
             refund_percent_before_cutoff, refund_percent_after_cutoff, 
             manual_confirmation, share_token)
        VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
        """,
        (request.user_id, title, description, category, int(duration), int(max_cap),
         pay_req, pay_amt, allow_res, allow_can, cut_hours, 
         ref_before, ref_after, manual, share_token),
        fetch="lastrowid",
    )
    appt = db.execute(
        "SELECT id, title, category, duration_mins, is_published, share_token FROM appointment_types WHERE id=%s",
        (appt_id,), fetch="one",
    )
...
def update_appointment(appt_id):
    ...
    db.execute(
        ...
        UPDATE appointment_types SET
            title=%s, description=%s, category=%s, duration_mins=%s, max_capacity=%s,
            payment_requirement=%s, payment_amount=%s, allow_rescheduling=%s,
            allow_cancellation=%s, cancellation_cutoff_hours=%s,
            refund_percent_before_cutoff=%s, refund_percent_after_cutoff=%s,
            manual_confirmation=%s
        WHERE id=%s
        """,
        (title, description, category, int(duration), int(max_cap),
         pay_req, pay_amt, allow_res, allow_can, cut_hours,
         ref_before, ref_after, manual, appt_id),
    )
    updated = db.execute(
        "SELECT id, title, description, category, duration_mins, max_capacity, payment_requirement, payment_amount, is_published, allow_cancellation, allow_rescheduling FROM appointment_types WHERE id=%s",
        (appt_id,), fetch="one",
    )
...
def delete_appointment(appt_id):
    ...
    if appt["organiser_id"] != request.user_id:
        return error("Forbidden", 403)
    # Delete related slots and questions first to avoid FK constraint errors
    db.execute("DELETE FROM appointment_questions WHERE appointment_type_id=%s", (appt_id,))
    db.execute("DELETE FROM slots WHERE appointment_type_id=%s", (appt_id,))
    db.execute("DELETE FROM appointment_types WHERE id=%s", (appt_id,))
...
def public_preview(token):
    appt = db.execute(
        ...
        SELECT at.id, at.title, at.description, at.duration_mins,
               ...
               at.allow_rescheduling, at.allow_cancellation,
               u.full_name AS organiser_name,
               pi.bio, pi.has_parking, pi.is_wheelchair_accessible, pi.noise_level
        FROM appointment_types at
        JOIN users u ON u.id = at.organiser_id
